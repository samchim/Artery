...
public class ARCloudAnchorExperienceManager : MonoBehaviour
{
    ...
    void Update() 
    {
        if(AllowCloudAnchorDelay)
        {
            if(timePassedAfterPlanesDetected <= maxScanningAreaTime)
            {
                ...
                arDebugManager.LogInfo($"Experience starts in {maxScanningAreaTime-timePassedAfterPlanesDetected} sec(s)");
            }
            else
            {
    ...
    {
        arDebugManager.LogInfo("Activate AR Cloud Anchor Experience");
        OnInitialized?.Invoke();
        Initialized = true;
        AllowCloudAnchorDelay = false;
        arPlaneManager.enabled = false;
    ...
    public void Restart()
    {
        arDebugManager.LogInfo("Restart AR Cloud Anchor Experience");
        OnRetarted?.Invoke();
        Initialized = false;
        AllowCloudAnchorDelay = true;
